...
def load_genome_sequence(filename):
    ...
    end_time = time.time()
    print(f"loaded genome sequence in {end_time - start_time:.2f} secs")
    return genome
...
def calculate_kmer_frequencies(sequence, k):
    ...
    for i in range(len(sequence) - k + 1):
        ...
        kmer_freqs[kmer] += 1
    end_time = time.time()
    print(f"calculated kmer frequencies in {end_time - start_time:.2f} secs")
    return kmer_freqs
...
def tokenize_file(genome_sequence, tokenizer):
    ...
    end_time = time.time()
    print(f"tokenized genome sequence in {end_time - start_time:.2f} secs")
...
def evaluate_vocab_size(kmer_freqs, genome_sequence, vocab_size, tokenizers_dir):
    ...
    end_time = time.time()
    print(f"Evaluated vocab size {vocab_size} in {end_time - start_time:.2f} seconds")
    return vocab_size, compression

@profile
def heuristic_search_optimal_vocab_size(kmer_freqs, genome_sequence, initial_vocab_size=50, step_size=1000, max_vocab_size=80000):
...
if __name__ == "__main__":
    ...
    optimal_vocab_size, optimal_compression_factor, compression_factors = heuristic_search_optimal_vocab_size(
        kmer_freqs, subsampled_sequence, initial_vocab_size=50, step_size=500, max_vocab_size=100000)
    end_time = time.time()
    print(f"search completed in {end_time - start_time:.2f} secs")

    print(f"optimal vocab size for {org}: {optimal_vocab_size}, compression factor: {optimal_compression_factor}")

    for vocab_size, compression_factor in compression_factors:
